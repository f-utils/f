...
class meta:
    ...
    def add(attribute, at, type_dict):
        ...
        if entity in entity_dict:
            ...
            raise meta.err(f"Entity '{entity}' not found.")
    ...
    def update(attribute, at, type_dict):
        ...
        if entity not in entity_dict:
            raise meta.err(f"Entity '{entity}' is not registered.")

        spec_info = entity_dict[entity]
        if attribute not in type_dict:
            raise meta.err(f"Attribute '{attribute}' is not allowed.")

        if attribute == 'description':
            def _update_desc_(new_description):
                if not isinstance(new_description, str):
                    raise meta.err("The new description must be a string.")
        ...
        if attribute == 'std' and 'spec' in spec_info:
            def _update_std_(new_standard_return):
                if not callable(new_standard_return):
                    raise meta.err("The new standard return must be a function or a lambda.")
                spec_info['spec']['std'] = {
        ...
        if attribute == 'body' and 'spec' in spec_info:
            def _update_body_(given_type, new_function):
                if not callable(new_function):
                    raise meta.err("The new body function must be a function or a lambda.")
        ...
        if attribute == 'tags':
            def _update_tag_(old_tag, new_tag):
                if not isinstance(old_tag, str) or not isinstance(new_tag, str):
                    raise meta.err("Both old tag and new tag must be strings.")
                if not spec_info['metadata']['tags']:
                    raise meta.err("Cannot update tag as the tags list is empty.")
                if new_tag in spec_info['metadata']['tags']:
                    raise meta.err(f"Tag '{new_tag}' already exists in entity '{entity}'.")
                if old_tag in spec_info['metadata']['tags']:
                    spec_info['metadata']['tags'][spec_info['metadata']['tags'].index(old_tag)] = new_tag
                else:
                    raise meta.err(f"Tag '{old_tag}' not found in entity '{entity}'.")
            return _update_tag_
        ...
        if attribute == 'comments':
            def _update_comment_(comment_id, new_comment_value):
                if not isinstance(comment_id, str) or not isinstance(new_comment_value, str):
                    raise meta.err("Both comment ID and new comment value must be strings.")
                if not spec_info['metadata']['comments']:
                    raise meta.err("Cannot update comment as the comments dictionary is empty.")
                if comment_id in spec_info['metadata']['comments']:
                    spec_info['metadata']['comments'][comment_id] = new_comment_value
                else:
                    raise meta.err(f"Comment ID '{comment_id}' not found in entity '{entity}'.")
            return _update_comment_

        raise meta.err(f"Unsupported attribute '{attribute}' for entity '{entity}'.")
    ...
    def check(names, at):
        for name in names:
            if not isinstance(name, str):
                raise meta.err(f"'{name}' is not a string.")
            if name not in at:
                raise meta.err(f"'{name}' is not an accessible entity.")
        return True
    ...
    def init(entity_name, description, std=None, at=None):
        ...
        if entity_name in entity_dict:
            raise meta.err(f"Entity '{entity_name}' is already registered.")
